...
class DALLEProvider(ImageProvider):
    """DALL-E 3 AI image generation provider."""
    
    def __init__(self):
        self.api_key = os.getenv("OPENAI_API_KEY", "")
    
    def search(self, keywords: List[str], count: int = 1) -> List[ImageSearchResult]:
        if not self.api_key:
        ...
        try:
            ...
            # Build concept from keywords
            concept = " ".join(keywords[:3])
            
            # DALL-E prompt for PPT illustrations
            prompt = f"""Create a clean, modern illustration for: {concept}

Style: Minimalist, professional, flat design with soft gradients.
Colors: Blues, teals, and accent colors.
NO text, NO words, NO letters in the image.
Simple and elegant visual metaphor."""
